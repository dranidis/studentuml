...
public class AddObjectFlowController extends AddEdgeController {
    ...
    protected void addFlow(NodeComponentGR src, NodeComponentGR trg, Point srcPoint, Point trgPoint) {
        ...
        if (src instanceof ActionNodeGR && !(trg instanceof ObjectNodeGR)) {
        ...
        if (trg instanceof ObjectNodeGR) {
            ...
            boolean allow = allowAddition(src.getComponent(), (ObjectNode) trg.getComponent());
            if (!allow) {
        ...
        ObjectFlow flow = new ObjectFlow(src.getComponent(), trg.getComponent());
        if (!guard.isEmpty()) {
            flow.setGuard(guard);
        }
    ...
    private boolean allowAddition(NodeComponent src, ObjectNode trg) {
        if (src instanceof ControlNode) {
            for (Edge edge : src.getIncomingEdges()) {
                src = edge.getSource();

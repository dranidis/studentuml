...
public class DCDLinkReconnectionTest {
    ...
    @Test
    public void testDependencyCannotReconnectToInterface() {
        ClassGR a = h.addClass("A");
        ClassGR b = h.addClass("B");
        InterfaceGR i = h.addInterface("I");
        ...
        // Dependencies are restricted to class-to-class only
        boolean canReconnect = dep.canReconnect(
                EndpointType.TARGET, i);
        assertFalse("Dependency should not allow interface target", canReconnect);

        boolean reconnected = dep.reconnectTarget(i);
        assertFalse("Reconnection to interface should fail", reconnected);
    }
